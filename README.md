...
4. 测试说明
    ...
    - 不同连接数，吞吐量会相差非常大，一般50-2000个连接会有相对较好的吞吐量，当只有一个连接时，多线程的威力发挥不出来，测试数据会比较难看^_^；当连接数达到几万时，吞吐量也会下降；各位可以对各种情况进行测试，然后进行综合对比，选择适合你的框架，不要盲目选择t-io作为你项目中的框架，因为t-io不是万能的。
